...
Range.CoordinationTarget=0                      ; 浮点数 , 距离目标多近时才会尝试进行协同攻击 , 小于 1 = 无限距离 , 默认值是 : 0 , 单位 : 格点
Range.CoordinationTargetMove=0                  ; 浮点数 , 精灵会移动到距离目标多近的位置 , 小于 1 = 无限距离 , 默认值是 : 0 , 单位 : 格点
                                                ; 通常情况下 , 建议此项的值略大于 Range.CoordinationTarget 来达到比较好的效果 , 当然其他值也是可以工作的 , 只是效果可能没那么好
Delay.CoordinationTarget=0                      ; 整数 , 协同攻击的开火间隔时间 , 小于 1 = 禁用 , 默认值是 0 , 单位 : 帧
Actions.CoordinationTarget=                     ; 武器动作列表 , 协同攻击时触发的武器动作 , 默认值是 空
```
...
Range.SurveilerTarget=0                         ; 浮点数 , 和目标保持的距离 , 小于 1 = 定在原地 , 默认值是 : 0 , 单位 : 格点
