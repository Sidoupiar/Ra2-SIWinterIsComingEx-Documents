...
Warp.AnimIn=                                    ; 动画类型 , 在传送出发位置播放的动画 , 默认值是 空
Warp.AnimOut=                                   ; 动画类型 , 在传送抵达位置播放的动画 , 默认值是 空
Warp.Self=no                                    ; yes/no , 翻转目标单位和攻击单位 , 即把攻击单位传送到目标单位处 , 默认值是 no
Warp.Force=no                                   ; yes/no , yes = 强制传送 , 不管目标位置是否可以容纳单位 , no = 传送到附近的可用位置 , 默认值是 no
Warp.ForceRange=3                               ; 浮点数 , Warp.Force=no 时 , 允许的可用位置距离目标位置的最远距离 , 小于 0 视为 0 处理 , 默认值是 3
...
Warp.ParaFloor=yes                              ; yes/no , 对于高度小于 Warp.ParaMinHeight 的单位 , 决定是否直接出现在地上 , 默认值是 yes
