...
```ini
[SomeWarheadType]
HouseVar.OperationType=None                     ; 操作类型 , 如 Add , Mult , Pow 等 , 可用操作和脚本中一致 , 默认值是 None
HouseVar.DataPack=                              ; 数据包的注册名 , 注意不是填写 IDCode , 数据包含义请见【上述列表给出的 AI 脚本动作】 , 没有数据包无法生效
HouseVar.Count=1                                ; 整数 , 生效多少次 , 默认值是 1
```
