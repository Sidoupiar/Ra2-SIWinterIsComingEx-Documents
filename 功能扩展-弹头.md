...
EnterScript.IgnoreRecruitable=yes               ; yes/no , 是否无视单位的可重组属性并强制把单位加入小队并执行指定的脚本 , 默认值是 yes
...
TargetEnterScript.IgnoreRecruitable=yes         ; yes/no , 是否无视单位的可重组属性并强制把单位加入小队并执行指定的脚本 , 默认值是 yes
...
PenetrateIronCurtainPercent=1                   ; 浮点数 , 穿透比例 , 负数 = 扣血变加血 (加血变扣血) , 默认值是 1
```
