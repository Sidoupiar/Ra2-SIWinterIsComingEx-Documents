...
```ini
[SomeWarheadType]
EXConvert=no                                    ; yes/no , 启用逻辑 , 默认值是 no
EXConvertFrom=                                  ; 单位列表 , 只给这些单位变形 , 不写就是任意单位
EXConvertTo=                                    ; 单位列表 , 变形为这些单位 , 随机选择 , 不写就无法变形 , 默认值是 空
EXConvertWeight=1                               ; 整数列表 , 选择变形结果的权重 , 小于 1 视为 1 处理 , 默认值是 1
EXConvertOnMap=yes                              ; yes/no , 要求仅当单位在地图上时才能进行变形 , 默认值是 yes
EXConvertOwner=no                               ; yes/no , 是否变为攻击者所属的作战方 , 默认值是 no
```

使用限制请见 [关于变形](/其他新类型/类型-单位升级设置.md#关于变形)。
