...
Effect.OffsetSourceRandom=no                    ; yes/no , 一般表示用于坐标或偏移量是否使用随机数 , 默认值是 no
Effect.OffsetTargetRandom=no                    ; yes/no , 一般表示用于坐标或偏移量是否使用随机数 , 默认值是 no
Effect.OffsetSame=no                            ; yes/no , 一般表示用于坐标或偏移量是否进行同步 , 默认值是 no
