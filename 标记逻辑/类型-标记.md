...
Active.Auto.Set=None                            ; 标记参数设置 , 自动激发时会合并此设置 , 默认值是 None (即无标记参数设置 , 不区分大小写)
...
After.NextMark.Set=None                         ; 标记参数设置 , 切换的标记在挂载时会合并此设置 , 不设置则使用标记的默认值 , 默认值是 None (即无标记参数设置 , 不区分大小写)
