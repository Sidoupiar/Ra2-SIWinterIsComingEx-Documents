...
Active.Auto.Setting=None                        ; 标记参数设置 , 自动激发时会合并此设置 , 默认值是 None (即无标记参数设置 , 不区分大小写)
...
After.NextMark.Setting=None                     ; 标记参数设置 , 切换的标记在挂载时会合并此设置 , 不设置则使用标记的默认值 , 默认值是 None (即无标记参数设置 , 不区分大小写)
                                                ; 当【After.Type=Next】时 , 标记参数设置中的 Duration = 0 表示继承当前标记的持续时间 (不设置则强制继承当前标记的持续时间)
