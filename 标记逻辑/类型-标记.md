...
AllowAIDifficulty=yes,yes,yes                   ; 三个 yes/no , 是否允许 AI 持有的单位挂载此标记 , 按难度区分 , 分别是【简单】【中等】【冷酷】 , 默认值是 yes,yes,yes
AllowGameMode=yes,yes                           ; 两个 yes/no , 是否允许在战役模式和其他模式 (遭遇战) 下挂载此标记 , 默认值是 yes,yes
...
DamageProcessType=Detonate                      ; 伤害处理类型 , 对于可以造成伤害的标记效果 , 是否造成真正的攻击伤害 , 不管如何设置都需要指定弹头 , 默认值是 Detonate (不区分大小写)
                                                ...
                                                ; Health = 直接扣除目标的生命值 , 而非造成实际的攻击伤害 , 此处理不做受击结算 , 不算做当前标记所属单位的攻击 , 不吃火力和防御等的加成
                                                ; 仅执行目标单位的标记的【自身处理阶段】 , 可以把目标扣死 , 届时会产生 1 真伤的死亡伤害 , 此伤害可以被免疫导致死不成
                                                ; 此属性对恢复和即死等效果造成的死亡伤害无效 , 对发射武器和 (检测) 条件引爆弹头等效果造成的间接伤害无效
                                                ; OnlyHealth = 类似于 Health , 但是不执行目标单位的标记的【自身处理阶段】
...
; 效果强度值相关属性
; 需要注意的是 , 这里的属性是用于定义【效果强度值】如何转化为【实际效果】的 , 标记的实际上的【效果强度值】由【标记参数设置】中的 Power 属性提供
...
; 广播与监听相关属性
; 需要标记处于生效阶段 , 但是并不是所有的效果种类都支持广播与监听
...
Digital.Primary=                                ; 数值显示设置 , 显示第 1 类数值 (显示哪些数值请见具体的标记效果) , 不设置不显示 , 默认值是 空
Digital.Secondary=                              ; 数值显示设置 , 显示第 2 类数值 (显示哪些数值请见具体的标记效果) , 不设置不显示 , 默认值是 空
Digital.Tertiary=                               ; 数值显示设置 , 显示第 3 类数值 (显示哪些数值请见具体的标记效果) , 不设置不显示 , 默认值是 空
Digital.Quaternary=                             ; 数值显示设置 , 显示第 4 类数值 (显示哪些数值请见具体的标记效果) , 不设置不显示 , 默认值是 空
Digital.AutoOffset=yes                          ; yes/no , 是否会根据标记的 Order 属性自动偏移 , 从而避免显示的数值和其他标记重叠在一起 , 偏移的高度由【数值显示设置】的 Size 属性决定 , 默认值是 yes
Digital.BufferSpeed=0.005                       ; 浮点数 , 【血条】的缓冲残影的消失速度 , 速度是每帧减少的百分比 , 0.005 意味着每帧减少 0.5% , 200 帧会掉空 , 默认值是 0.005 , 单位 : 比例/帧
```

另见 [类型-标记参数设置](/其他新类型/类型-标记参数设置.md#类型-标记参数设置)、[类型-判断单位条件](/其他新类型/类型-判断单位条件.md#类型-判断单位条件)、[类型-坐标运动轨迹](/其他新类型/类型-坐标运动轨迹.md#类型-坐标运动轨迹) 和 [类型-数值显示设置](/其他新类型/类型-数值显示设置.md)。

效果强度值说明另见 [概念：效果强度值叠加](/标记逻辑/总体说明.md#概念效果强度值叠加) 和 [概念：效果强度值转换](/标记逻辑/总体说明.md#概念效果强度值转换)。  
广播与监听说明另见 [概念：广播与监听](/标记逻辑/总体说明.md#概念广播与监听)。