...
Digital.SameLine=yes                            ; yes/no , 当队列类型一致时 , 第 1 2 3 4 类数值是否显示在同一行 , 默认值是 yes
Digital.LineSpacing=5                           ; 整数 , 显示在同一行时 , 第 1 2 3 4 类数值之间的间距 , Digital.SameLine=yes 时为横向间距 , Digital.SameLine=no 时为纵向间距 , 默认值是 5
