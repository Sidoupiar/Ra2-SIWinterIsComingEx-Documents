...
; 不是原有属性 , 但是为了成套加上的
; 【武器索引】从 0 开始 , 注意不要超出虚拟武器的数量 , 不然单位会变得不正常甚至弹窗
OverpoweredWeapon=-1                            ; 整数 , 被充能时使用的【武器索引】 , -1 = 使用默认的被充能武器 (副武器) , 默认值是 -1
AssaultWeapon=-1                                ; 整数 , 充能时使用的【武器索引】 , -1 = 使用默认的充能武器 (副武器) , 默认值是 -1
...
Weapon.Overpowered.Number=0                     ; 整数 , 被充能时使用的【武器编号】 , 负数或 0 = 使用默认的被充能武器 (副武器) , 默认值是 0
...
WeaponCount
Weapon.Virtual.Count
OpenTransportWeapon
DeployFireWeapon
NoAmmoWeapon
OverpoweredWeapon
Weapon.Normal.Count
Weapon.OpenTransport.Number
Weapon.OpenTransport.Count
Weapon.DeployFire.Number
Weapon.DeployFire.Count
Weapon.NoAmmo.Number
Weapon.NoAmmo.Count
Weapon.Overpowered.Number
Weapon.Overpowered.Count