...
EXP.Provide=0                                   ; 浮点数 , 提供的原始经验值 , 可以是负数 , 但是负数太多可能导致单位升不了级 , 并且负数会使攻击者的经验值增加 , 0 = 使用原始伤害值 , 默认值是 0 , 单位 : 点
...
EXPPool.Provide=0                               ; 浮点数 , 提供的原始经验值 , 可以是负数 , 但是负数太多可能导致单位升不了级 , 并且负数会增加池中的经验值总量 , 0 = 使用原始伤害值 , 默认值是 0 , 单位 : 点
