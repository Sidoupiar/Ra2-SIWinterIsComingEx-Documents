...
; 对旧经验系统的升级行为的支持
Insignia.Rookie=                                ; shp 文件名称 , 新兵等级时显示的军衔 , 不含后缀名 , 作为 [SomeTechnoType] -> Insignia.Rookie 的默认值使用 , 默认值是 pip (留空会不显示)
Insignia.Veteran=                               ; shp 文件名称 , 老兵等级时显示的军衔 , 不含后缀名 , 作为 [SomeTechnoType] -> Insignia.Veteran 的默认值使用 , 默认值是 pip (留空会不显示)
Insignia.Elite=                                 ; shp 文件名称 , 精英等级时显示的军衔 , 不含后缀名 , 作为 [SomeTechnoType] -> Insignia.Elite 的默认值使用 , 默认值是 pip (留空会不显示)
InsigniaFrame.Rookie=-1                         ; 整数 , 新兵等级时显示的军衔图像帧 , 负数 = 不显示 , 作为 [SomeTechnoType] -> InsigniaFrame.Rookie 的默认值使用 , 默认值是 -1
InsigniaFrame.Veteran=14                        ; 整数 , 老兵等级时显示的军衔图像帧 , 负数 = 不显示 , 作为 [SomeTechnoType] -> InsigniaFrame.Veteran 的默认值使用 , 默认值是 14
InsigniaFrame.Elite=15                          ; 整数 , 精英等级时显示的军衔图像帧 , 负数 = 不显示 , 作为 [SomeTechnoType] -> InsigniaFrame.Elite 的默认值使用 , 默认值是 15
