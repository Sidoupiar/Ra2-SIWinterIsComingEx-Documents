...
; 可驻军建筑分配经验值 , 写可驻军建筑上
EXP.Occupy=no                                   ; yes/no , 可驻军建筑是否可以获得经验值 , 作为 [SomeTechnoType] -> EXP.Occupy 的默认值使用 , 默认值是 no
...
EXP.Occupant=no                                 ; yes/no , 驻军步兵是否可以获得经验值 , 作为 [SomeTechnoType] -> EXP.Occupant 的默认值使用 , 默认值是 no
