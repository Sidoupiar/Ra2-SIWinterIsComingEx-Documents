...
而且单位需要拥有 `Trainable=yes` 才能获取到用于分配的原始经验值，行为会被 `EXP.Alloc` 影响。

位于 `rulesmd.ini`：

```ini
; 经验值分配规则
EXP.Alloc=no                                    ; yes/no , 单位获得的经验值是否进行经验值分配 , 默认值是 [EXPControls] -> EXP.Alloc 的值
                                                ; 对于 Trainable=yes 的单位 , 不影响自身获得的经验值
                                                ; 对于 Trainable=no 的单位 , 虽然自身无法获得经验值 , 但是此项可以影响相关的单位是否可以获得经验值
