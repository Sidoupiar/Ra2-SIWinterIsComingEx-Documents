...
|3|经验分配部分除了 `EXP.OldPercent`、`EXP.OldCost`、`EXP.OldRatio`、`EXP.OldOwner` 使用最新的单位类型的设置以外，其余的均使用原始的单位类型的设置。|
|4|单位类型的 `Insignia.Old` 使用最新的单位类型的设置，`Insignia.OldLevel` 使用原始的单位类型的设置。|
