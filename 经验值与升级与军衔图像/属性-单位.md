...
; 复用已有的属性
Insignia.ShowEnemy=yes                           ; yes/no , 敌对作战方是否可以看到军衔 (看不到则会隐藏所有的军衔) , 默认值是 [EXPControls] -> Insignia.OldShowEnemy 的值
...
Insignia.WeaponX=                               ; shp 文件名称 , Gunner=yes 使用第 X 个武器时显示的军衔 , 不含后缀名 , 默认值是 空 (即不显示)
Insignia.WeaponX.Rookie=                        ; shp 文件名称 , 新兵等级时 Gunner=yes 使用第 X 个武器时显示的军衔 , 不含后缀名 , 默认值是 Insignia.WeaponX 的值
Insignia.WeaponX.Veteran=                       ; shp 文件名称 , 老兵等级时 Gunner=yes 使用第 X 个武器时显示的军衔 , 不含后缀名 , 默认值是 Insignia.WeaponX 的值
Insignia.WeaponX.Elite=                         ; shp 文件名称 , 精英等级时 Gunner=yes 使用第 X 个武器时显示的军衔 , 不含后缀名 , 默认值是 Insignia.WeaponX 的值
InsigniaFrame.WeaponX=-1                        ; 整数 , Gunner=yes 使用第 X 个武器时显示的军衔图像帧 , 默认值是 -1 (即默认帧 , 新兵等级 = -1 , 老兵等级 = 14 , 精英等级 = 15)
InsigniaFrame.WeaponX.Rookie=-1                 ; 整数 , 新兵等级时 Gunner=yes 使用第 X 个武器时显示的军衔图像帧 , 默认值是 InsigniaFrame.WeaponX 的值
InsigniaFrame.WeaponX.Veteran=-1                ; 整数 , 老兵等级时 Gunner=yes 使用第 X 个武器时显示的军衔图像帧 , 默认值是 InsigniaFrame.WeaponX 的值
InsigniaFrame.WeaponX.Elite=-1                  ; 整数 , 精英等级时 Gunner=yes 使用第 X 个武器时显示的军衔图像帧 , 默认值是 InsigniaFrame.WeaponX 的值
InsigniaFrames.WeaponX=-1,-1,-1                 ; 三个整数 , 总体设定 3 个等级时 Gunner=yes 使用第 X 个武器时显示的军衔图像帧 , 默认值是 -1,-1,-1 (即默认帧 , 新兵等级 = -1 , 老兵等级 = 14 , 精英等级 = 15)
```
