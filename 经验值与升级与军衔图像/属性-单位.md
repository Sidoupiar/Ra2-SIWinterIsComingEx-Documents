...
Exp.ShareRange=768                              ; 整数 , 分享经验值的范围 , 默认值是 [EXPControls] -> Exp.ShareRange 的值 , 单位 : 格点
...
Exp.ShareSelfPriorOnly=yes                      ; yes/no , 当击杀者满级后分享经验值时是正常分享 , 还是把自己的那一份也拿出来 (也就是会分享出去两份经验值) , 需要 Exp.ShareSelfPrior=yes
                                                ; 默认值是 [EXPControls] -> Exp.ShareSelfPriorOnly 的值
Exp.ShareSelfLevel=0                            ; 整数 , 当击杀者达到这个等级后会视为自己已经 "满级" 并允许分享经验给周围的单位 , 需要 Exp.ShareSelfPrior=yes
                                                ; 0 = 实际上的满级 , 负数 = 差多少级到实际上的满级 , 默认值是 [EXPControls] -> Exp.ShareSelfLevel 的值
