...
经验来源一共有 7 种：击杀、国家能力、训练所、间谍渗透、弹头提供、Buff 提供、经验值分配，其中经验值分配较为特殊，它不是一个独立的来源，因此它会和其他 6 种经验来源混合出现。  
除了 `Allow.Alloc` 以外，经验来源行为仅对单位自己生效，不会影响经验值分配或其他的可以传递经验值的功能（比如弹头的消耗开火单位自己的经验值的这部分功能）。
...
Allow.Alloc=yes                                 ; yes/no , 是否允许经验值分配 , 来源为经验值分配的经验值是无法再次被分配的 , 默认值是 yes
