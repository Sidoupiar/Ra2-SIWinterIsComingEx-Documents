...
Up.Buffs=                                       ; 增益列表 , 单位升级到此等级时会挂载这些增益 , 默认值是 空
Up.BuffSets=                                    ; 增益参数设置列表 , 对应的增益在挂载时会合并此设置 , 不设置则使用增益的默认值
Up.BuffCheck=                                   ; 判断单位条件 , 条件判断 , 需要满足所有的条件 , 单位自己判自己
Up.BuffRemoves=                                 ; 增益列表 , 单位升级到此等级时会移除这些增益 , 默认值是 空
...
Down.Buffs=                                     ; 增益列表 , 单位降级到此等级时会挂载这些增益 , 默认值是 空
Down.BuffSets=                                  ; 增益参数设置列表 , 对应的增益在挂载时会合并此设置 , 不设置则使用增益的默认值
Down.BuffCheck=                                 ; 判断单位条件 , 条件判断 , 需要满足所有的条件 , 单位自己判自己
Down.BuffRemoves=                               ; 增益列表 , 单位升级到此等级时会移除这些增益 , 默认值是 空
...
另见 [类型-军衔图像设置](/其他新类型/类型-军衔图像设置.md#类型-军衔图像设置)、[类型-增益参数设置](/其他新类型/类型-增益参数设置.md#类型-增益参数设置)、[类型-判断单位条件](/其他新类型/类型-判断单位条件.md#类型-判断单位条件)。
