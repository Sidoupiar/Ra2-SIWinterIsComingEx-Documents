...
; 数据属性
TheseMarks=                                     ; 标记列表 , 投放者向投放出来的单位挂载的标记 , 不写就不挂载 , 投放者通常的小队的队长
ThoseMarks=                                     ; 标记列表 , 投放出来的单位向投放者挂载的标记 , 不写就不挂载
MarkSets=                                       ; 标记参数设置列表 , 目标标记会合并此设置 , 按顺序排列 , 前 N 项对应 TheseMarks 的标记 , 后 M 项对应 ThoseMarks 的标记
