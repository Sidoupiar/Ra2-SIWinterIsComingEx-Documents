...
; 数据属性
TheseBuffs=                                     ; 增益列表 , 投放者向投放出来的单位挂载的增益 , 不写就不挂载 , 投放者通常的小队的队长
ThoseBuffs=                                     ; 增益列表 , 投放出来的单位向投放者挂载的增益 , 不写就不挂载
BuffSets=                                       ; 增益参数设置列表 , 目标增益会合并此设置 , 按顺序排列 , 前 N 项对应 TheseBuffs 的增益 , 后 M 项对应 ThoseBuffs 的增益
