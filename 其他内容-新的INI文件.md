...
由于一些难以处理的问题， WIC 定义了一套新的 INI 文件，这些文件主要目标是让 WIC 的扩展类型使用起来更加简单（只有显式标注为 `扩展类型` 的类型才算扩展类型）。



## 文件位置

1. 需要把这些 INI 文件放在和 dll 同级的名为 `WICSettings` 的文件夹内，内部的 INI 文件均会被读取（包括子文件夹）。

2. 中文路径可能会产生问题，以后会尝试解决。
