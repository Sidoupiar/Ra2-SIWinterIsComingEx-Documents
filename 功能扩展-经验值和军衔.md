...
EnemyInsignia=yes                               ; yes/no , 敌对作战方是否可以看到军衔 (看不到则会隐藏所有的军衔) ， 也作为 [SomeTechnoType] -> Insignia.ShowEnemy 的默认值使用 , 默认值是 yes
DefaultInsigniaSet=DefaultInsignia              ; 军衔图像设置 , 显示的军衔的默认值 , 默认值是 DefaultInsignia (即会自动载入 [DefaultInsignia] 作为此项的值)
```

举个栗子：  
如果 VeteranCap=2，则单位最多可以获得 2 级的经验量，当单位升级到精英级后（等级 2），如果吃到负数经验，那么他会立刻掉回老兵级（等级 1）；  
如果 VeteranCap=3，则单位最多可以获得 3 级的经验量，当单位升级到精英级后，他依然可以继续获得经验（虽然不会继续升级），吃满经验后，如果吃到不满 1 级的负数经验，那么会因为他的经验量仍然高于精英级的要求而保持在精英级。
