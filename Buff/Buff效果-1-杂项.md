...
* 【生产率】不是整数时可能会导致最终扣除的金额不是整数，小数部分会进位计算。  
...
|参数|说明|
|:-:|:-|
|【生产量】|每次最多吃下多少个格子的矿物（实际数量四舍五入），0 = 一个格子都没，小于 0 按 0 算，默认值是 `0`，单位：格子。|
|【生产率】|生产矿物时扣钱的比例，【生产率】不为 `0` 时没钱无法生产，负数 = 获得钱，1 = 100%，0.2 = 20%，0 = 0%，默认值是 `0`。|
|【每个格子的最大生产量】|每个格子最多生产这么多矿物（无四舍五入），负数 = 移除矿物（需要原本有矿物），0 = 无法生产矿物，默认值是 `0`。|
