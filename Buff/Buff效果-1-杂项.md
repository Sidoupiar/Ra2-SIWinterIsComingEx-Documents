...
每一项都是【造价倍率】：  
负数 = 变成加钱，0 = 免费，0.2 = 20% 造价，1 = 100% 造价，默认值是 `0`。  
单位原本的造价会因为【造价倍率】而改变。
...
每一项都是【建造数量限制】：  
负数 = 不限制，0 = 造不了，正数 = 固定数值（去尾），默认值是 `0`，单位：次。  
...
* 技能会在施放的一瞬间把成本扣除（包括预定时）。

### 效果强度值转换：1+M+N+K+L 项参数

|参数|说明|
|:-:|:-|
|【技能预定数量】|最多可以积累这么多个预定，0 = 1 个预定，小于 0 按 0 算，默认值是 `0`，单位：个。|
...
每一项都是【冷却时间】：  
负数 = 固定时间（实际取绝对值），0 = 瞬间完成，正数 = 固定时间，默认值是 `0`，单位：帧。  
技能的实际冷却时间最少为 1 帧。

后续的 K 个参数项数与 `Effect.AcceptBuffs` 的长度相匹配。  
每一项都是【施放数量限制】：  
负数 = 不限制，0 = 施放不出来，正数 = 固定数值（去尾），默认值是 `0`，单位：次。  
