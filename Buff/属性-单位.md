...
[SomeTechnoType]
; 以下这两个列表的元素数量需要相匹配
BuffToCarrier.Types=                            ; Buff 列表 , 子机给航母挂载的 Buff , 默认值是 空
BuffToBunkered.Types=                           ; Buff 列表 , 坦克碉堡给碉堡内坦克挂载的 Buff , 默认值是 空
BuffToLocomotorVictim.Types=                    ; Buff 列表 , IsLocomotor 武器 ( 磁电坦克 ) 所有者给被抬起来单位挂载的 Buff , 默认值是 空
BuffToController.Types=                         ; Buff 列表 , 被心控单位给心控者挂载的 Buff , 默认值是 空
BuffToControlled.Types=                         ; Buff 列表 , 心控单位给被心控单位挂载的 Buff , 默认值是 空
```
