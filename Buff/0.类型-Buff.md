...
MultiInstance=no                                ; yes/no , 挂载这个 Buff 时 , 每个 Buff 都会单独存在而非相互合并 , 默认值是 no
                                                ; 注意 : 这是特殊用途的解决方案 , 此类 Buff 应该用作纯效果 Buff 而非其他用途 (栗如跳板 , 触发器 , 指示器 , 火力分配等用途)
                                                ; 部分情况下可能导致 "是否挂载此种 Buff 的判定" 不准确的问题 , 并且可能存在潜在的性能问题
...
Active.Auto.CheckFail.Type=Continue             ; 自动激发判断失败处理类型 , 当自动激发失败时 , 会按照这里定义的类型执行后续操作 (不区分大小写) , 默认值是 Continue
