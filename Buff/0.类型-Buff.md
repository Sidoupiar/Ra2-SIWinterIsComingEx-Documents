...
[SomeBuffType]
; 【挂载】状态控制相关属性
Mark.Type=Merge                                 ; 挂载类型 , 具有相同类型的 Buff 挂载在同一个单位身上时它们会合并 , 此项可以改变这个行为 , 默认值是 Merge (不区分大小写)
                                                ; Merge   = 与原来的 Buff 合并 (也就是原来的行为)
                                                ; Replace = 干掉原来的 Buff , 重新挂载
                                                ; Next    = 使用新的 Buff 类型再次尝试挂载 (新的 Buff 类型如果也已存在且也是 Mark.Type=Next 则会再次更换 , 请注意死循环)
                                                ; Multy   = 多个实例并存 , 部分情况下可能导致 "是否挂载此种 Buff 的判定" 不准确的问题 , 并且可能存在潜在的性能问题
                                                ;           注意 : 这是特殊用途的解决方案 , 此类 Buff 应该用作纯效果 Buff 而非其他用途 (栗如跳板 , 触发器 , 指示器 , 火力分配等用途)
                                                ; None    = 放弃挂载 , 什么都不做
Mark.NextBuffs=                                 ; Buff 列表 , 当【Mark.Type=Next】时要切换为这些 Buff , 如果为空则效果等同于 None
Mark.NextBuff.Sets=                             ; Buff 参数设置列表 , 切换的 Buff 在挂载时会合并这些设置 , 顺序与 Mark.NextBuffs 相对应 , 不设置则使用 Buff 的默认值
```
...
[SomeBuffType]
; 【激发】状态控制相关属性
Active.Auto=0                                   ; 整数 , 获取 Buff 后经过多久就会自动激发 , 小于 0 视为不自动激发 , 0 = 立即进入激发状态 , 默认值是 [BuffControls] -> Default.Active.Auto 的值 , 单位 : 帧
...
Active.Auto.CheckDelay=31                       ; 整数 , 每次判断的时间间隔 ， 小于等于 1 = 每帧检测一次 (注意性能) , 默认值是 31
Active.Auto.CheckFail.Type=Continue             ; 自动激发判断失败处理类型 , 当自动激发失败时 , 会按照这里定义的类型执行后续操作 , 默认值是 Continue (不区分大小写)
                                                ; Continue      = 继续判断条件 , 在等待 Active.Auto.CheckDelay 这么久之后继续尝试激发
                                                ; ContinueMerge = 同 Continue , 并且给 Buff 合并 Active.Auto.CheckFail.NextBuff.Sets 的第一项 Buff 参数设置
                                                ; After         = 变成结束状态
                                                ; Remove        = 移除此 Buff
                                                ; Next          = 切换至新的 Buff (旧的会被移除)
Active.Auto.CheckFail.NextBuffs=                ; Buff 列表 , 当【Active.Auto.CheckFail.Type=Next】时要切换为这些 Buff , 如果为空则效果等同于 Remove
Active.Auto.CheckFail.NextBuff.Sets=            ; Buff 参数设置列表 , 切换的 Buff 在挂载时会合并这些设置 , 顺序与 Active.Auto.CheckFail.NextBuffs 相对应 , 不设置则使用 Buff 的默认值
...
[SomeBuffType]
; 【结束】状态控制相关属性
After.Type=After                                ; 准备进入结束状态时 , Buff 何去何从 (具体激发时长取决于效果种类 , 不区分大小写) , 默认值是 After
                                                ; After     = 变成结束状态
...
After.NextBuff.Sets=                            ; Buff 参数设置列表 , 切换的 Buff 在挂载时会合并这些设置 , 顺序与 After.NextBuffs 相对应 , 不设置则使用 Buff 的默认值
...
Effect.Sounds=                                  ; 声音列表 , 需要注册
Effect.EVAs=                                    ; EVA 语音列表 , 需要注册
...
Effect.DelayAppend=0                            ; 整数 , 小于 0 按 0 算 , 默认值是 0
...
另见 [类型-Buff 参数设置](/Buff/3.子类型-Buff参数设置.md#子类型-Buff-参数设置)、[类型-判断单位条件](/Buff/3.子类型-判断单位条件.md#子类型-判断单位条件)、[类型-坐标运动轨迹](/Buff/3.子类型-坐标运动轨迹.md#子类型-坐标运动轨迹)、[类型-数值显示设置](/数值显示/类型-数值显示设置.md)、[类型-按钮显示设置](/数值显示/类型-按钮显示设置.md) 和 [空白快捷键](/功能扩展-空白快捷键.md)。

效果强度值说明另见 [概念：效果强度值叠加](/Buff/0.总体说明.md#概念效果强度值叠加) 和 [概念：效果强度值转换](/Buff/0.总体说明.md#概念效果强度值转换)。  
广播与监听说明另见 [概念：广播与监听](/Buff/0.总体说明.md#概念广播与监听)。
