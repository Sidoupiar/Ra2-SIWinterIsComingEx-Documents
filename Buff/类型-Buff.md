...
AllowExist=yes,yes,yes                          ; 三个 yes/no , Buff 在单位【转换】【进入载具】【进入建筑驻军】时是否保留 (均无法恢复) , 不写或留空使用默认值 , 默认值是 yes,yes,yes
...
EnableCodeDamage=yes                            ; yes/no , Buff 是否会忽视 (不处理) 拥有 IgnoreDefenses=yes 标识的伤害 (通常是各种机制杀的伤害) , 当 Buff 拥有伤害处理相关的处理阶段时才有效 , 亡语不受此控制 (效果会正常触发)
...
Digital.BufferSpeed=0.005                       ; 浮点数 , 【血条】的缓冲残影的消失速度 , 速度是每帧减少的百分比 , 0.005 意味着每帧减少 0.5% , 200 帧会掉空 , 默认值是 0.005 , 单位 : 比例/帧
```

另见 [类型-Buff 参数设置](/其他新类型/类型-Buff参数设置.md#类型-Buff-参数设置)、[类型-判断单位条件](/其他新类型/类型-判断单位条件.md#类型-判断单位条件)、[类型-坐标运动轨迹](/其他新类型/类型-坐标运动轨迹.md#类型-坐标运动轨迹) 和 [类型-数值显示设置](/其他新类型/类型-数值显示设置.md)。
