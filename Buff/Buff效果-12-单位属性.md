...
|`Effect.Type=Prop.MultEXPCost`|倍率：经验值需求|`除法`形式的 `MultEXP`，此值为 0 时会强制变为 0.1%。另见 [获取经验值](/经验值与升级与军衔图像/属性-单位.md#获取经验值)。|
...
|`Effect.Type=Prop.ExMultEXPCost`|累乘：经验值需求|由于精度问题，会放弃绝对值小于 1% 的因数。`除法`形式的 `MultEXP`，另见 [获取经验值](/经验值与升级与军衔图像/属性-单位.md#获取经验值)。|
...
|`Effect.Type=Prop.AddEXPCost`|追加：经验值需求|单位升级所需的经验值，正数会让单位升级更加困难，负数会让单位升级更加简单，另见 [提供经验值](/经验值与升级与军衔图像/属性-单位.md#提供经验值)。|
...
|`Effect.Type=Prop.FinalEXPCost`|最终：经验值需求|和追加一样，是加算。单位升级所需的经验值，正数会让单位升级更加困难，负数会让单位升级更加简单，另见 [提供经验值](/经验值与升级与军衔图像/属性-单位.md#提供经验值)。|
