...
ProductionFactory.Building=1                    ; 浮点数 , 产能工厂的效果 , 建筑单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
ProductionFactory.Defense=1                     ; 浮点数 , 产能工厂的效果 , 防御单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
ProductionFactory.Infantry=1                    ; 浮点数 , 产能工厂的效果 , 步兵单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
ProductionFactory.Unit=1                        ; 浮点数 , 产能工厂的效果 , 载具单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
ProductionFactory.Aircraft=1                    ; 浮点数 , 产能工厂的效果 , 飞机单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
ProductionFactory.Naval=1                       ; 浮点数 , 产能工厂的效果 , 海军单位的生产时间变为原来的多少倍 , 多个产能工厂效果乘算 , 不能是负数 , 默认值是 1
...
; 生产速度倍率
BuildTime.Production=1                          ; yes/no , 单位建造时间受到【产能工厂】影响的倍率 , 默认值是 1
BuildTime.ProductionTypes=                      ; 整数列表 , 单位建造时间受到这些【产能工厂的自定义产能类型】的影响 , 彼此之间乘算 , 默认值是 空
