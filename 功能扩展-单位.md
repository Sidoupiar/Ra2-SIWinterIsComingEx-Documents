...
DrainMoneyFrameDelay=30                         ; 整数 , 飞碟吸取矿厂时 , 每次吸取的时间间隔 , 不能小于 1 , 默认值是 [CombatDamage] -> DrainMoneyFrameDelay 的值
DrainMoneyAmount=30                             ; 整数 , 飞碟吸取矿厂时 , 每次吸取的金额 , 可以是负数 , 默认值是 [CombatDamage] -> DrainMoneyAmount 的值
DrainMoneyAmount.Self=0                         ; 整数 , 等同于 DrainMoneyAmount , 但是是写在被吸取的单位上的 , 默认值是 0
```
