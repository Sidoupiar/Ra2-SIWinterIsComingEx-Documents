...
如果遇到增益不发挥效果的情况，并且确定是没有载入的话，请反馈。
...
[SomeBuffType]
; 基础属性
Order=0                                         ; 整数 , 增益的处理顺序 , 值越小越靠前 , 值相同则顺序取决于挂载的时间 , 挂载越早越靠前 , 可以是负数 , 默认值是 0
                                                ; 在处理增益的生效效果的过程中 , 仅当增益的处理阶段类型与当前处理阶段相匹配时 (<> 中的字段 , 逗号分隔) , 处理顺序才有效 , 增益仅在列出的处理阶段中才会参与处理流程
AllowExist=yes,yes,yes                          ; 三个 yes/no , 增益在单位【转换】【进入载具】【进入建筑驻军】时是否保留 (均无法恢复) , 不写或留空使用默认值 , 默认值是 yes,yes,yes
                                                ; 【转换】包括展开或收起 , 还有类似多段升级的这种 , 暂不支持武直这种类型的转换
AllowPlayer=yes                                 ; yes/no , 是否允许玩家持有的单位挂载此增益 , 默认值是 yes
AllowAI=yes                                     ; yes/no , 是否允许 AI 持有的单位挂载此增益 , 中立阵营也算 AI 控制的 , 默认值是 yes
AllowAIDifficulty=yes,yes,yes                   ; 三个 yes/no , 是否允许 AI 持有的单位挂载此增益 , 按难度区分 , 分别是【简单】【中等】【冷酷】 , 默认值是 yes,yes,yes
AllowGameMode=yes,yes                           ; 两个 yes/no , 是否允许在战役模式和其他模式 (遭遇战) 下挂载此增益 , 默认值是 yes,yes
EnableCodeDamage=yes                            ; yes/no , 增益是否会忽视 (不处理) 拥有 IgnoreDefenses=yes 标识的伤害 (通常是各种机制杀的伤害) , 当增益拥有伤害处理相关的处理阶段时才有效 , 亡语不受此控制 (效果会正常触发)
                                                ; yes = 忽视这样的伤害 , 默认值是 yes
WaitingIronCurtain=no                           ; yes/no , 增益是否会在单位处于铁幕效果下时失效 , 铁幕消失后恢复效果 , 不影响下方的各种动画 , 默认值是 no
                                                ; yes = 当单位处于铁幕效果下时 , 增益进入生效状态后会被冻结 , 效果不会生效 , 冷却时间不恢复 , 弹头不炸 , 伤害不处理 , 亡语不触发等等
LockOwnerHouse=no                               ; yes/no , 作战方是否固定为挂载瞬间的作战方 , yes = 维持挂载瞬间的作战方 , no = 当单位被心控时作战方也会变化 , 默认值是 no
                                                ; 这里有个特殊情况 , 当挂载瞬间的作战方被击败后 , 此项强制为 no
DamageProcessType=Detonate                      ; 伤害处理类型 , 对于可以造成伤害的增益效果 , 是否造成真正的攻击伤害 , 不管如何设置都需要指定弹头 , 默认值是 Detonate (不区分大小写)
                                                ...
                                                ; Health = 直接扣除目标的生命值 , 而非造成实际的攻击伤害 , 此处理不做受击结算 , 不算做当前增益所属单位的攻击 , 不吃火力和防御等的加成
                                                ; 仅执行目标单位的增益的【自身处理阶段】 , 可以把目标扣死 , 届时会产生 1 真伤的死亡伤害 , 此伤害可以被免疫导致死不成
                                                ; 此属性对恢复和即死等效果造成的死亡伤害无效 , 对发射武器和 (检测) 条件引爆弹头等效果造成的间接伤害无效
                                                ; OnlyHealth = 类似于 Health , 但是不执行目标单位的增益的【自身处理阶段】
...
Anim.Removed=None,None,None,None                ; 四个动画 , 增益被移除时显示的动画 , 对应【挂载】【激发】【生效】【结束】四个状态 , 不写或留空使用默认值 , 默认值是 None (即无动画 , 不区分大小写)
Anim.End=None,None,None,None                    ; 四个动画 , 增益挂载持续时间结束时显示的动画 , 对应【挂载】【激发】【生效】【结束】四个状态 , 不写或留空使用默认值 , 默认值是 None (即无动画 , 不区分大小写)
Anim.Kill=None,None,None,None                   ; 四个动画 , 增益生命值清零时显示的动画 , 对应【挂载】【激发】【生效】【结束】四个状态 , 不写或留空使用默认值 , 默认值是 None (即无动画 , 不区分大小写)

; 挂载持续时间相关属性
Duration=-1                                     ; 整数 , 默认的增益的挂载持续时间 , 仅当挂载增益时输入的时间为 0 时生效 , 负数 = 永久 , 0 = 挂不上 , 默认值是 -1 , 单位 : 帧
                                                ; 对于非永久的增益 , 挂载持续时间的硬上限是 10e 帧 , 无法超过这个值 , 挂载持续时间在 10e 的范围内可以一直叠加 , 但是无法叠加到【无限】这个状态
Duration.Max=None                               ; 整数 , 每次挂载增益时 (叠加挂载持续时间时) 允许的挂载持续时间上限 , 负数或 None 或 N = 不限制 , 默认值是 None (不区分大小写) , 单位 : 帧
Duration.Min=None                               ; 整数 , 每次挂载增益时 (叠加挂载持续时间时) 允许的挂载持续时间下限 , 负数或 None 或 N = 不限制 , 默认值是 None (不区分大小写) , 单位 : 帧
                                                ; 上下限仅对非永久的增益有效

; 生命值相关属性
Health=-1                                       ; 浮点数 , 默认的增益的生命值 , 仅当挂载增益时输入的生命值为 0 时生效 , 负数 = 无敌 , 0 = 一碰就凉 , 默认值是 -1 , 单位 : 点
                                                ; 对于非无敌的增益 , 生命值的硬上限是 10e 点 , 无法超过这个值 , 生命值在 10e 的范围内可以一直恢复 , 但是无法恢复到【无敌】这个状态
Health.Max=None                                 ; 浮点数 , 调整增益的生命值时 (恢复生命值时) 允许的生命值上限 , 负数或 None 或 N = 不限制 , 默认值是 None (不区分大小写) , 单位 : 点
Health.Min=None                                 ; 浮点数 , 调整增益的生命值时 (恢复生命值时) 允许的生命值下限 , 负数或 None 或 N = 不限制 , 默认值是 None (不区分大小写) , 单位 : 点
                                                ; 上下限仅对非无敌增益有效
Health.Damage=no                                ; yes/no , 单位受到伤害时增益是否也会被伤害 , 默认值是 no
                                                ; 增益使用单位的护甲 , 且在受伤阶段处理完毕后才会结算生命值 , 增益生命值清零时会被立刻移除

; 状态控制相关属性
Mark.Multy=yes                                  ; yes/no , 挂载期间是否允许重复挂载
Active.Auto=-1                                  ; 整数 , 获取增益后经过多久就会自动激发 , 小于 0 视为不自动激发 , 0 = 立即进入激发状态 , 默认值是 [BuffControls] -> Default.Active.Auto 的值 , 单位 : 帧
Active.Auto.Set=                                ; 增益参数设置 , 自动激发时会合并此设置
Active.Auto.Check=                              ; 判断单位条件 , 条件判断 , 需要满足所有的条件才会自动激发 , 默认值是 空
                                                ; 满足 Active.Auto 之后才会判断 , Active.Auto=0 时在挂载后会立刻判断一次
                                                ; 判断时来源单位是增益的来源单位 , 目标单位是被挂载增益的单位自己
Active.Auto.CheckDelay=31                       ; 整数 , 每次判断的时间间隔 ， 负数和 0 和 1 = 每帧检测一次 (注意性能) , 默认值是 31
Active.Before=0                                 ; 整数 , 挂载增益后多久才允许激发 , 小于 0 视为 0 处理 , 默认值是 0 , 单位 : 帧
Active.After=0                                  ; 整数 , 激发结束后多久才允许重复激发 , 小于 0 视为 0 处理 , 默认值是 0 , 单位 : 帧
Active.Delay=0                                  ; 整数 , 激发后多久才会生效 , 生效前增益过期的话就没有效果了 , 0 = 立即进入生效状态 , 小于 0 视为 0 处理 , 默认值是 0 , 单位 : 帧
Active.Multy=no                                 ; yes/no , 激发期间是否允许重复激发
                                                ; 若增益允许重复激发 , 则激发时附带的效果强度值会相互叠加 , 调整增益的效果强度值类效果不受此限制 , 单纯叠效果强度值不能激发增益 , 不激发便没有实际效果
After.Type=After                                ; 准备进入结束状态时 , 增益何去何从 (具体激发时长取决于效果种类 , 不区分大小写)
                                                ; After     = 变成结束状态 , 默认值是 After
                                                ; Reset     = 变成挂载状态 , 可以重新激发 , 经过 1 帧后才会切换 (硬限制)
                                                ; Remove    = 移除此增益
                                                ; Next      = 切换至新的增益 (旧的会被移除)
                                                ; AfterNext = 切换至新的增益 (旧的会被移除) , 但是是在挂载持续时间结束之后
After.NextBuffs=                                ; 增益列表 , 当【After.Type=Next】或【After.Type=AfterNext】时要切换为这些增益 , 如果为空则效果等同于 Remove
                                                ; 若填写当前增益类型则等同于没填写 (即无法递归挂载 , 因为挂载持续时间会叠加而非挂载一个新的同类增益)
After.NextBuff.Sets=                            ; 增益参数设置列表 , 切换的增益在挂载时会合并这些设置 , 顺序与 After.NextBuffs 相对应 , 不设置则使用增益的默认值
                                                ; 当【After.Type=Next】时 , 增益参数设置中的 Duration = 0 表示继承当前增益的挂载持续时间 (不设置则强制继承当前增益的挂载持续时间)

; 效果种类相关属性
Effect.Type=None                                ; 生效时的效果种类 (只能填写一个) , 不同种类效果不同 , 具体见下 (不区分大小写) , 默认值是 None

; 以下属性是增益所支持的 , 但是具体作用取决于 Effect.Type 的值 , 没提到的属性意味着在对应的效果种类中无效
Effect.AcceptBuffs=                             ; 增益列表
Effect.ExceptBuffs=                             ; 增益列表
Effect.Technos=                                 ; 单位列表
Effect.Weapons=                                 ; 武器列表
...
; 效果强度值相关属性
; 需要注意的是 , 这里的属性是用于定义【效果强度值】如何转化为【实际效果】的 , 增益的实际上的【效果强度值】由【增益参数设置】中的 Power 属性提供
...
; 广播与监听相关属性
; 需要增益处于生效阶段 , 但是并不是所有的效果种类都支持广播与监听
...
Digital.Primary=                                ; 数值显示设置 , 显示第 1 类数值 (显示哪些数值请见具体的增益效果) , 不设置不显示 , 默认值是 空
Digital.Secondary=                              ; 数值显示设置 , 显示第 2 类数值 (显示哪些数值请见具体的增益效果) , 不设置不显示 , 默认值是 空
Digital.Tertiary=                               ; 数值显示设置 , 显示第 3 类数值 (显示哪些数值请见具体的增益效果) , 不设置不显示 , 默认值是 空
Digital.Quaternary=                             ; 数值显示设置 , 显示第 4 类数值 (显示哪些数值请见具体的增益效果) , 不设置不显示 , 默认值是 空
Digital.AutoOffset=yes                          ; yes/no , 是否会根据增益的 Order 属性自动偏移 , 从而避免显示的数值和其他增益重叠在一起 , 偏移的高度由【数值显示设置】的 Size 属性决定 , 默认值是 yes
Digital.BufferSpeed=0.005                       ; 浮点数 , 【血条】的缓冲残影的消失速度 , 速度是每帧减少的百分比 , 0.005 意味着每帧减少 0.5% , 200 帧会掉空 , 默认值是 0.005 , 单位 : 比例/帧
```

另见 [类型-增益参数设置](/其他新类型/类型-增益参数设置.md#类型-增益参数设置)、[类型-判断单位条件](/其他新类型/类型-判断单位条件.md#类型-判断单位条件)、[类型-坐标运动轨迹](/其他新类型/类型-坐标运动轨迹.md#类型-坐标运动轨迹) 和 [类型-数值显示设置](/其他新类型/类型-数值显示设置.md)。

效果强度值说明另见 [概念：效果强度值叠加](/增益/总体说明.md#概念效果强度值叠加) 和 [概念：效果强度值转换](/增益/总体说明.md#概念效果强度值转换)。  
广播与监听说明另见 [概念：广播与监听](/增益/总体说明.md#概念广播与监听)。